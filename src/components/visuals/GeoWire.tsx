...
/* ---------- Hilfsfunktionen (AUßERHALB der Komponente, stabil) ---------- */

const rotXYZ = ([x,y,z]: number[], rx: number, ry: number, rz: number) => {
  const cx=Math.cos(rx), sx=Math.sin(rx);
  const cy=Math.cos(ry), sy=Math.sin(ry);
  const cz=Math.cos(rz), sz=Math.sin(rz);
  const X = x*cz - y*sz, Y = x*sz + y*cz, Z = z; // Z
  const Y2 = Y*cx - Z*sx, Z2 = Y*sx + Z*cx;     // X
  const X3 = X*cy + Z2*sy, Z3 = -X*sy + Z2*cy;  // Y
  return [X3, Y2, Z3];
};
...
export default function GeoWire({
  ...
  // pick ist stabil (keine Abhängigkeiten mehr notwendig)
  const pick = useCallback((k: ShapeKey) => {
  ...
  // pathsFor ist ebenfalls stabil
  const pathsFor = useCallback(
    (shape: { v:number[][]; e:[number,number][] }, rx:number, ry:number, rz:number) => {
  ...
  useEffect(() => {
    ...
    const _aWrap = A_wrap.current!, aBack = A_backPath.current!, aFront = A_frontPath.current!;
    const _bWrap = B_wrap.current!, bBack = B_backPath.current!, bFront = B_frontPath.current!;
    ...
    let t = 0;
    let raf = 0;
    const rxBase = 0.35, ryBase = 0.0, rzBase = 0.0;
  ...
  }, [accent, strength, shiftXvw, reduce, shape, pick, pathsFor]);

  // Public switch
  useEffect(() => {
    const now = Date.now();
    if (now - (lastSwitchAtRef.current || 0) < 120) return;
